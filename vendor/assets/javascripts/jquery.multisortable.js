...
!function($){
	...
	$.fn.multiselectable = function(options) {
		...
		return this.each(function() {
			...
			if (!list.children().data('multiselectable')) {
				...
				// Modified code for single image drag functionality
				list.children().on("mousedown", function(e) {	
	...
	$.fn.multisortable = function(options) {
		...
		return this.each(function() {
			var list = $(this) // the ul this function has been called on
			
			//enable multi-selection function on the ul
			...
			options.stop = function(event, ui) {
				...
				// ******** IMPORTANT - MIKE HEU MODIFIED PLUGIN ********* //
				// Had to alter plugin to look for dropped_in_menu global variable - regroup was affecting cancel
				
				if(window.dropped_on_menu == false){
